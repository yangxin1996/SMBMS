...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/provider/delprovider.action",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/view.action?proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/modify.action?proid="+ obj.attr("proid");
	});
